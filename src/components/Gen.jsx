...
import { SCHEMES, WORKOUTS } from '../utils/swold'
    ...
    return(
        ...
        <div className='flex flex-col gap-4'>
            <div className='flex items-center gap-2 justify-center'>
                ...
                <h4 className='text-xl sm:text-2xl med:text-3xl'>{title}</h4>
...
export default function Gen(props) {
  ...
  return (
    <SectionWrap id={'generate'} header={"Crea tu workout"} 
    title={['Abraza el', 'Dolor', ]}>
        <Header index={'01'} title={'Elegir musculos'}
        description={'seleciona tu workout'}/>
        ...
        <Header index={'02'} title={'Marcar objetivo'}
        description={'Seleciona musculos'}/>
        <div className='bg-slate-950 border flex flex-col border-solid border-blue-400 rounded-lg'>

            <button onClick={toggleModal} className='relative p-3  flex items-center justify-center'>
                <p className='capitalize'>{muscles.length == 0 ? 'Select muscle groups' : muscles.join(' ')}</p>
                <i className="fa-solid fa-down-long top-1/2 -translate-y-1/2 absolute right-3"></i>
            </button>
        ...
        <Header index={'03'} title={'tipo de workout'} description={"Seleciona tu ultimo objetivo."} />
